...
const DECEL_SPEED = 5
const BACKWARDS_MAX_SPEED = 2000
...
func _physics_process(delta: float) -> void:
	...
	if Input.is_action_pressed("up") and not stunned:
		...
		velocity.y = move_toward(velocity.y, 0, DECEL_SPEED) # moves velocity towards 0, slowing it down every from by 10 when not pressing up or down
